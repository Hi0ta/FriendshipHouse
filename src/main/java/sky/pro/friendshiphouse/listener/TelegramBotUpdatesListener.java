...
//далее распознавание сообщений от друзей/усыновителей
                ...
                } else if (textIncomingMessage.matches("(отчет)([А-Яа-яA-Za-z0-9\\p{P}\s)]+)")) { //TODO как принять слово "отчет" без учета регистра - где и как правильно вписать "i" !?
                    Report report = new Report();
                    report.setReportStatus(AWAITING_VERIFICATION);
                    report.setReportDate(LocalDate.now());
                    report.setReportMessage(update.message().text().substring(5));
                    report.setAdopter(adopterService.getAdopterByChatId(chatId));
                    reportService.createReport(report);//TODO создать если прислан за эту дату впервые и заменить если прислан повторно
                ...
                } else if (textIncomingMessage == null) { //TODO поменять условие
                    byte[] photo = update.message().photo()[1].fileId().getBytes();  //TODO написать метод который принимает картинку  -этот написан неправильно
                    if (reportService.editReportPhoto(chatId, photo)) { //если true
                        SendMessage reportPhotoMessage = new SendMessage(chatId, "Фото принято и отправлено на проверку волонтеру.");
                        telegramBot.execute(reportPhotoMessage);
                    }
                    SendMessage reportNotFound = new SendMessage(chatId, "Сначала отправь текст отчета.");
